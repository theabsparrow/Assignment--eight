...
const BuyTheBook = ({ allbook }) => {

    const { image, bookName, author, category, yearOfPublishing, tags, totalPages, rating, publisher, bookId } = allbook;
                    ...
                    <div className='font-work lg:leading-[19px] flex flex-col lg:flex-row justify-between gap-4 lg:gap-0 mt-6'>
                        ...
                        <div>
                            <Link to={`/allbook/${bookId}`}>
                                <button className='btn bg-[#23BE0A] text-white text-xl font-semibold leading-[19px] border-none'>Buy Now</button>
                            </Link>
                        </div>
                    </div>
                </div>
...
BuyTheBook.propTypes = {
    buyABook: PropTypes.object
}
BuyTheBook.propTypes = {
